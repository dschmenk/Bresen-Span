...
```
Bam! That's it. You will find the complete routines and a hacked together test harness in the source directory. A real-mode DOS binary (VGA required) can be run inside DOSBox to see it in action with some timing. Depending on what hardware/emulation you use, the differences between the normal and fast lines may not look like much. A few reasons for this discrepency are that the span drawing routines are horribly unoptimized and are actually slower than the single pixel drawing for slopes near 1.0. Also, on real hardware with a fast CPU and a slow I/O bus, the writes to the VGA will saturate the I/O bus and limit how fast the algorithm runs, regardless of implementation. Try replacing the drawing routines with empty, dummy routines to compare the actual speed of the line algorithms.
